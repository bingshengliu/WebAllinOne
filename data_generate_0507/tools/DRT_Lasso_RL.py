...
def cvxopt_solve_qp(H0, c):
    ...
    #            max_iter=200000, eps_prim_inf=1E-5, eps_dual_inf=1E-5, solver=cp.OSQP)
    #
    # gamma = x.value
    ...
    #     'abs_tol': 1e-10,  # 绝对误差
    #     'rel_tol': 1e-10,  # 相对误差
    #     'feas_tol': 1e-10,  # 可行性误差
    #     'show_progress': True  # 显示进度
    # })
    N_out = c.shape[1]
    h = matrix(np.zeros(N_out))
    G = matrix(-np.identity(N_out))
    sol = solvers.qp(matrix(H0*1000), matrix(c.T*1000), G, h)
    gamma = np.array(sol['x']).flatten()
...
def main(data, lambda0, coeff, Reg_type):
    ...
    x_ridge = cvxopt_solve_qp(H_combined, f_combined)
